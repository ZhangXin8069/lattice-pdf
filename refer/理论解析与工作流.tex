...
需要计算的三点关联函数为：
\begin{equation}
C_3(\Delta t; z, P_z) = \braket{\Omega |
\mathcal{N}(\mathbf{P}, t_{\text{sink}}) \;
O(z; t_{\text{op}}) \;
\bar{\mathcal{N}}(\mathbf{P}, t_{\text{src}})
| \Omega}
\end{equation}
其中 $\mathcal{N}$ 是质子插值算符，$O(z)$ 是胶子 quasi-PDF 算符。

对于胶子算符，只有 \textbf{disconnected 图}（quark-disconnected）有贡献：
\begin{center}
\begin{tikzpicture}[scale=1.2]
  % 质子传播子 (两条水平线表示)
  \draw[very thick] (0,1) -- (4,1) node[midway, above] {$N(t_{\text{sink}})$};
  \draw[very thick] (0,0) -- (4,0) node[midway, below] {$\bar{N}(t_{\text{src}})$};
  % 胶子算符 (垂直方向)
  \draw[very thick, decorate, decoration={coil, aspect=0.5, amplitude=3pt}]
    (2,1.5) -- (2,0.3);
  \node at (2.5,1.2) {$O(z; t_{\text{op}})$};
  % 断开标记
  \draw[fill=white] (1.5,0.9) rectangle (2.5,1.1);
  \draw[fill=white] (1.5,-0.1) rectangle (2.5,0.1);
\end{tikzpicture}
\end{center}

\textbf{关键近似}：三点关联函数解耦为质子两点函数和 OPE 算符的乘积：
\begin{equation}\label{eq:factorization}
\boxed{C_3(\Delta t; z, P_z) \;\approx\; C_2(\Delta t; P_z) \cdot \vev{O(z)}_{\text{gauge}}}
\end{equation}
其中：
\begin{itemize}
  \item $C_2(\Delta t; P_z) = \vev{\mathcal{N}(t_{\text{sink}}) \bar{\mathcal{N}}(t_{\text{src}})}$ 为质子两点关联函数；
  \item $\vev{O(z)}_{\text{gauge}}$ 为纯规范场中 OPE 算符的真空期望值。
\end{itemize}

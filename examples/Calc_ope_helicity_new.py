...
if rank in {0, 1, 2}:
    ...
    # np.save(
    #     "%s/ops_mu%d_nu%d_dz%d_conf%s%s.npz"
    ...
    for _dz in range(delta_z):
        ...
        ops[:, :, :, _dz] = operators_new_z0_mu2_xy(
            gauge, zdir, pla, pla_tilde, _dz, _mu, _nu, _mu2, _nu2, Nt, Nx
        )
        ops_mz[:, :, :, _dz] = operators_new_z0_mz_mu2_xy(
            gauge, zdir, pla, pla_tilde, _dz, _mu, _nu, _mu2, _nu2, Nt, Nx
        )
    ed = time.time()
    print("calculate op at mu:%d, nu:%d done, time used: %.3f s" % (_mu, _nu, ed - st))
    ...
    np.save(
        "%s/ops_minus_mu%d_nu%d_dz%d_conf%s%s.npy"
        % (output_dir, _mu, _nu, delta_z, conf_id, sysm),
        ops_mz,
    )

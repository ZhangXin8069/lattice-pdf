...
ope_dir = "/public/group/lqcd/donghx/Ope_Gluon/Result_hpy_4D_10times/beta6.308_mu-0.2510_ms-0.2170_u00.859727_u0s0.954467_L32x64/xdir/"
# sysm = ""
sysm = "_AS"
...
# for conf in range(11000, 60300, 50):
# for conf in range(2700, 9860, 50):
for conf in range(10000, 18000, 10):
    if (
        # os.path.exists("%s/%s/ops_pla_conf%s.npz" % (ope_dir, conf, conf))
        # and
        os.path.exists(
            "%s/%s/ops_mu3_nu2_dz24_conf%s%s.npy" % (ope_dir, conf, conf, sysm)
        )
        # and os.path.exists(
        #     "%s/%s/ops_mu1_nu2_dz24_conf%s%s.npy" % (ope_dir, conf, conf, sysm)
        # )
